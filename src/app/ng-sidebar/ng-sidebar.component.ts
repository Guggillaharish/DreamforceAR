...
export class NgSidebarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    $(".navbar-left__toggle").click(function(e) {
        e.preventDefault(), $(this).parents("nav").toggleClass("navbar-expand")
        ...
        e.preventDefault(), $this = $(this).next(), $(".side-menu>li").removeClass("active"), $(this).parent().addClass("active"), $(".side-menu__dropdown-menu").not($this).slideUp(), $(this).next().slideToggle()
    }), $(".drop-user-menu").click(function(e) {
        e.preventDefault(), $(window).width() < 768 && $(this).next().slideToggle()
